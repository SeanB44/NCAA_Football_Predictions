...
cor(mod.dat$difference,mod.dat$Win_pct_diff_exp)#No use
...
cor(mod.dat$difference, mod.dat$sp_plus_diff)#0.77
...
cor(mod.dat$difference, mod.dat$postgame_wp_diff)#0.51 (Better but not great)
...
cor(mod.dat$difference, mod.dat[,(ncol(mod.dat)-3):ncol(mod.dat)])
...
cor(mod.dat$difference, mod.dat$returning_ppa_diff)# 0.35
# # no
...
mod.dat<-mod.dat%>%
  select(-c(Home_PwrRk_home, SOS_PwrRk_home, Last5_PwrRk_home,Pwr_Rk_home,
            Away_PwrRk_away, SOS_PwrRk_away, Last5_PwrRk_away, Pwr_Rk_away))
...
cor(mod.dat$difference,mod.dat$return_recruit_portal_home)
cor(mod.dat$difference,mod.dat$return_recruit_portal_away) # not good
mod.dat$return_recruit_portal_diff<-mod.dat$return_recruit_portal_home-mod.dat$return_recruit_portal_away
cor(mod.dat$difference,mod.dat$return_recruit_portal_diff)
...
cor(mod.dat$difference, mod.dat$Predictive_PwrRk_diff)
...
names(mod.dat)
mod.dat$home_win<-NULL #do not need difference, only binary W/L
...
###### MODEL 1v2: RAW DATA, ALL VARIABLES ######
set.seed(109)
train<-sample(1:nrow(mod.dat), nrow(mod.dat)*.667, replace = F)
test<-which(!c(1:nrow(mod.dat)) %in% train)
...
v2fit.1 <- lm(difference ~ `spread_home`+`Predictive_PwrRk_diff`+`elo_diff`+`fpi_diff`+`sp_plus_diff`, data = v2model1.dat[,-c(1:5)])
summary(v2fit.1) #rsq=0.745
...
# Total Absolute Difference
sum(abs(v2mod1.test.dat$predicted-v2mod1.test.dat$difference)) #1160
# Mean Absolute Difference
mean(abs(v2mod1.test.dat$predicted-v2mod1.test.dat$difference)) #8.47
...
sum(v2mod1.test.dat$pred_cover_correct)/nrow(v2mod1.test.dat) #73.0%
...
mod2.best.bic
mod2.best.adjr2
mod2.best.cp == mod2.best.adjr2

# Next model is all variables in all or in 2
new_var_list_1<-c("spread_home","Predictive_PwrRk_diff","win_pct_diff","elo_diff",
                  "fpi_diff", "sp_plus_diff",         
                  "postgame_wp_diff","AP_away"  
) 
...
# Mean Absolute Difference
mean(abs(mod3.test.dat$predicted-mod3.test.dat$difference)) #8.49
...
## Model 4: Model on new full Variable list
